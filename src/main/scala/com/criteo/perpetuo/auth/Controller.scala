...
class LocalUsersRetrievingController @Inject()(appConfig: AppConfig, permissions: Permissions, jwtEncoder: JWTEncoder) extends BaseController {
  private val localUserNames = appConfig.config.tryGetStringList("auth.localUserNames").getOrElse(Set()).toSet
