...
class JenkinsExecutionSpec extends Test {
  ...
  test("Execution's host, job name and build id are correctly parsed") {
    ...
    val execution2 = new JenkinsExecution(jenkinsConfig, "http://build.criteois.lan/job/test-executor/18")
    ...
    val localExecution = new JenkinsExecution(jenkinsConfig, "http://localhost:8080/job/perpetuo-executor/1")
  ...
  test("Wrong href generates an error") {
    val exc = the[IllegalArgumentException] thrownBy new JenkinsExecution(jenkinsConfig, "http://build.criteois.lan/job/test")
