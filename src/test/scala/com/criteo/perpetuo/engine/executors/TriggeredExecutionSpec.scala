...
class TriggeredExecutionSpec extends Test {
  ...
  test("The TriggeredExecutionFinder can load a Rundeck execution") {
    finder(ShallowExecutionTrace(42, Some("https://rundeck.somewhere/project/foo-bar/execution/show/42"), Some("https://rundeck.somewhere/project/foo-bar/execution/show/42"), ExecutionState.completed, "")).stopper shouldBe defined
  }

  test("The TriggeredExecutionFinder fails to load an execution of an unknown type") {
    val exc = the[RuntimeException] thrownBy finder(ShallowExecutionTrace(51, Some("https://idont-kn.ow/what.im?doing"), Some("https://idont-kn.ow/what.im?doing"), ExecutionState.running, ""))
